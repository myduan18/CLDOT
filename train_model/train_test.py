...
from model.drp_model.CLDOT import CLDOT
...
def total_loss(out_dict, ic50,  
    ...
    # common_features, specific_features, disc_output, disc_labels = out_dict['disc_features']

    # from torch.nn import functional as F
    # adv_loss = F.cross_entropy(disc_output, disc_labels)
    # ortho_loss_val = orthogonal_loss(common_features, specific_features)

    # adv_weight = 0.05
    # if epoch > 50:
    #     adv_weight = min(0.05, adv_weight + 0.001)
...
def get_batch_fracs(batch_smiles_list):
    ...
    indices = []
    count = 0
    for drug_fracs in fracs_list: # 遍历每个药物的碎片列表
        e_d_indices = []
        for f in drug_fracs:
...
def train_multi_view_model(args, train_set, val_set, test_set, i, cell_name_dict, drug_name_dict, log_file, path):
    ...
    # substruct_para = {
    #     'num_layer': 2, 'emb_dim': 64, 'graph_pooling': 'mean',
