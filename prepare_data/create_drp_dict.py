...
def n_fold_split(type='mix', drug_name = None, cell_name = None, drug_response_dict = None, seed = 1234, n_folds = 10, test_portion = 0.1): ## type: mix, cb, db
   random.seed(seed)
   ...
   if type == 'mix':
        ...
        random.seed(seed)
        np.random.seed(seed)
        num_total = len(drug_response_dict) # 100
        indices = np.arange(num_total)
        np.random.shuffle(indices)
        test_size = int(num_total * test_portion) # 10
        train_val_size = num_total - test_size # 90
        test_idx = indices[:test_size] # [:10]
        train_val_indices = indices[test_size:] # [10:]
        fold_size = train_val_size // n_folds # [90//10]=9
        train_dict = { } 
        val_dict = {}
        for i in range(5):
            validation_indices = train_val_indices[i * fold_size:(i+1) * fold_size] # [0:10%] [10%-20%]
