...
def arg_parse():
    ...
    parser.add_argument('--seed',type = int, default=2048 )#1234
    ...
    parser.add_argument('--cl_weight', type = float, default= 1e-3) # GDSC 5e-3, CCLE 1e-2
    ...
    parser.add_argument('--graph_embedding_dim', type=int, default='128', help='output embedding dim') 
    ...
    # parser.add_argument('--cell_desc_path', type=str, default=root+ '/CCLE/cell_descriptions.json', help='cell description path')
...
if __name__ == '__main__':
    ...
    train_dict, val_dict, test_dict = n_fold_split(drug_name=drug_name, cell_name= cell_name, 
                                                   drug_response_dict= drug_response_dict, 
                                                    ...
                                                    seed = args.seed, test_portion=0.2)
    i = args.part
    ...
    # with open('./evaluation/drug_response_nan_dict.pkl','rb') as f:
    #     drug_response_nan_dict = dill.load(f)

    # test_nan_set = graph_drp_nan_dataset(drug_response_nan_dict, data, batch_size=args.batch_size)
    # test_nan_loader = graph_drp_loader(test_nan_set,batch_size= args.batch_size,shuffle = True)


    # for batch in test_nan_loader:
    #     print(batch['cell_line'])

    # with open('test_nan_loader.pkl','wb') as f:
    #     dill.dump(test_nan_loader, f)

    # ipdb.set_trace()


    # print('test_nan_loader.pkl saved!')

    ########################################################################
    

    log_file = 'loss_log_test.txt'

    model_save_path = f'./save_model/save_model_test.pth' # contrastive loss 1e-3
