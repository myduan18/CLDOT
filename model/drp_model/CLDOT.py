...
class CLDOT(torch.nn.Module):
    ...
    def __init__(
        ...
        self.predictor = nn.Sequential(
            ...
            nn.BatchNorm1d(512),  
            nn.ELU(),
            nn.Linear(512, 64), # only text view
            nn.BatchNorm1d(64),  
            nn.ELU(),
            nn.Linear(64, 1)
        ...
        #     nn.Linear(512+128, 64), # only text view
        #     # nn.Linear(512, 64), # only text view
        ...
        # self.node_cell_linear = nn.Linear(7964, 64) # 8046
        self.node_drug_linear = nn.Linear(512, 128)

        self.cell_text_linear = nn.Linear(768, 128)
        self.drug_text_linear = nn.Linear(768, 128)
        ...
        self.cell_linear = nn.Sequential(
            ...
            nn.Linear(omic_para['gen_dim'], 512),
            nn.BatchNorm1d(512), 
            nn.ELU(),
        ...
        if self.use_regulizer_cell == 'True':
            ...
            self.cell_regulizer = nn.Linear(128, omic_para['cell_dim']) ## 26 cancer types 128
    ...
    def forward(self, batch,
        ...
        # node_protein_embeddings = cell_expression.unsqueeze(2) * self.cell_protein_embedding.unsqueeze(0)
        # node_protein_embeddings = node_protein_embeddings.sum(dim=1)
        # node_protein_embeddings = self.BatchNorm(node_protein_embeddings)

        node_embeddings = cell_expression

        # ipdb.set_trace()

        x_dict = {

            # 'cell_line': self.node_cell_linear(batch['cell_line'].x.to(device)),
            'cell_line': node_embeddings,
        ...
        x_exp = self.cell_linear(gen_feats.reshape(b, -1)) # 128-> 64
        
        x_mut = self.mut_selfA(mut_feats) # [64]
        x_cnv = self.cnv_selfA(cnv_feats) # [64]
        ...
        cell_text_embedding = self.cell_text_linear(self.cell_texts_embeddings[batch_cell_indices])
        drug_text_embedding = self.drug_text_linear(self.drug_texts_embeddings[batch_drug_indices])
        ...
        views = [tower_view, graph_view, text_view]
        ...
        if self.use_regulizer_cell == 'True':
            cell_class = self.cell_regulizer(x_exp)
        ...
        return {
            'pred': prediction, 'cell_regulizer':cell_class, 'drug_regulizer': drug_class, 'drug_pathway':drug_pathway,
